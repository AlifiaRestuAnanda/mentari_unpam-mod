...
;(function () {
  ...
  function injectNotesUI() {
    ...
    const storageKey = 'catatan_chrome_' + courseTitle;

    window.addEventListener('message', (event) => {
        if (event.source !== window || !event.data.type) {
            return;
        }
        if (event.data.type === 'NOTE_DATA' && event.data.key === storageKey) {
            noteTextarea.value = event.data.value || '';
        }
        if (event.data.type === 'NOTE_SAVED_CONFIRMATION' && event.data.success) {
    ...
    function loadCatatan() {
        window.postMessage({ type: 'GET_NOTE', key: storageKey }, '*');
    }

    saveButton.addEventListener('click', function() {
        const catatanTeks = noteTextarea.value;
        window.postMessage({ type: 'SET_NOTE', key: storageKey, value: catatanTeks }, '*');
    });
